...
const MobileCarousel = ({ data, cardType }) => {
  ...
  const onTouchEnd   = (e) => {
    ...
    // Prevent browser from firing synthetic click after touch
    e.preventDefault();
    ...
    <div className="flex items-center gap-3 w-full">
      ...
      <div className="flex-shrink-0 w-10">
        {index > 0 && (
          <button
            onClick={goPrev}
            className="flex items-center justify-center w-10 h-10 rounded-xl border-2 border-[#f0c417] text-[#f0c417] bg-[#111111] hover:bg-[#f0c417]/10 active:bg-[#f0c417]/20 transition-all duration-200 shadow-[0_4px_12px_rgba(240,196,23,0.15)]"
      ...
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="bg-white/[0.03] border border-white/[0.02] border-l-4 border-l-[#f0c417] rounded-lg p-5 flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.15)] cursor-pointer select-none"
          >
            <div className="flex flex-col gap-3">
              <h3 className="text-lg font-bold leading-snug text-[#f0c417] font-[Inter]">
                {item.title}
              </h3>
              <div className={`relative overflow-hidden rounded-lg h-44 w-full ${isBlog ? "bg-[#1a1a2e]" : "bg-[#1a2a4a]"} flex items-center justify-center`}>
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.parentElement.innerHTML = `<span class="text-gray-400 font-medium text-sm">${item.fallbackText}</span>`;
                  }}
                />
              </div>
                ...
                {isOpen && (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 pt-2">
                      {item.tags.map((tag) => (
                        <span key={tag} className="bg-[#f0c417]/10 border border-[#f0c417]/35 text-[#f0c417] rounded-full px-3 py-1 text-xs font-semibold tracking-wide">{tag}</span>
                      ))}
                    </div>
                  </motion.div>
                )}
            ...
            <div className="flex justify-center gap-1.5 mt-4">
              {data.map((_, i) => (
                <span
                  key={i}
                  className={`inline-block rounded-full transition-all duration-300 ${i === index ? "w-5 h-1.5 bg-[#f0c417]" : "w-1.5 h-1.5 bg-white/25"}`}
                />
              ))}
            </div>
          </motion.div>
...
const Blogs = () => {
  ...
  const cardBase = "bg-white/[0.03] border border-white/[0.02] border-l-4 border-l-[#f0c417] rounded-lg p-5 md:p-6 flex flex-col justify-between transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.15)] hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(240,196,23,0.15)] active:scale-[0.99]";
  ...
  return (
    <div className="career-bg-pattern min-h-screen text-white font-[Inter] relative overflow-hidden">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-20 sm:pt-24 md:pt-28 pb-10 md:pb-20">
        <h2
          className="text-3xl sm:text-4xl md:text-6xl font-black text-center text-[#f0c417] mb-10 md:mb-16 tracking-tight"
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          {/* RIGHT arrow — forward (page 1 only) */}
          <div className="flex-shrink-0 w-11">
